...
def test_adds_new_node_after_a_given_item():
    ...
    l1.add_after(target_node_data=8, new_node=Node(4))

    assert l1.__repr__() == "7 -> 33 -> 19 -> 8 -> 4 -> 0 -> 11"


def test_adds_new_node_before_a_given_item():
    l1 = LinkedList(nodes=[7, 33, 19, 8, 0, 11])
    l1.add_before(target_node_data=8, new_node=Node(4))

    assert l1.__repr__() == "7 -> 33 -> 19 -> 4 -> 8 -> 0 -> 11"
...
def test_raises_when_adding_a_node_before_an_item_when_list_is_empty():
    l1 = LinkedList(nodes=[])
...
def test_raises_when_adding_a_node_before_an_item_when_item_is_not_present_in_list():
    l1 = LinkedList(nodes=[1, 2, 3])
